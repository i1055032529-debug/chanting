...
Godot 像素风餐厅经营游戏。已实现可继续购置和自由摆放餐桌、逐格扩建、四道菜、独立烹饪小游戏、多员工协作与工资，以及跨日经营、食材采购、菜单定价和设备升级。
...
- 初始四张桌子，有空位和金币即可继续购买；各桌有独立订单与耐心。顶部订单卡每页显示五张桌，可翻页；卡片显示菜品、状态和剩余秒数，不足 12 秒会变红。
...
- 开店前可自由摆放所有餐桌与座椅、两处烹饪台、出餐台和餐盘回收台；新餐桌没有固定数量上限，受可用空间和金币限制。烹饪设备升级使主角与员工的制作速度提高 25%。布局和升级跨日保留。详见 [阶段 4.5 实现说明](docs/stage-4-5.md)。
- 开店前进入“逐格扩建”，直接点击餐厅场景中高亮的相邻地块，立即铺地；可持续向右、向下决定扩建形状，没有预设的空地表格或右侧、下方墙体。地板与上方墙面使用小尺寸图片图块重复铺设；可平移镜头布置家具，营业时镜头跟随主角。详见 [逐格扩建说明](docs/stage-4-6-grid.md)。
