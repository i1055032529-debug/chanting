...
开局有一名员工，默认自动做菜、上菜、收盘和清洁。员工出发前会领取任务；同一份菜、餐桌或炉灶只会分配给一方。顶部显示员工当前工作和失败原因。按 M 打开管理面板，关闭某项工作或调整优先级；员工会先做完手上的事，再应用新设置。禁用整名员工后，主角仍可独立完成营业日。首位员工的正式雇佣、工资和跨日管理安排在阶段四。
...
- 出餐台一次只能放一份菜；每个角色一次只能拿一件物品。把菜送错桌会提示，食物不会丢失。
