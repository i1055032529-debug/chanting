...
func _run() -> void:
	...
	check(Layout.customer_route(fifth, fifth[-1] + Vector2(100, 25)).size() > 0, "customer route reaches a new table through the walkable floor")
	var sixth_position := Layout.free_table_position(fifth)
	var sixth: Array[Vector2] = fifth.duplicate()
	sixth.append(sixth_position)
	check(sixth_position != Vector2.ZERO and Layout.valid(sixth), "a sixth table is allowed when a valid space exists")
	...
	paged._toggle_layout()
	paged._begin_layout_preview(true)
