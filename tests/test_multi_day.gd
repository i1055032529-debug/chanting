...
func _run() -> void:
	var model = Day.new()
	check(model.phase == "preopen" and model.day_number == 1 and model.coins == 0, "new run starts before day one")
	model.advance(30.0)
	check(model.elapsed == 0.0 and not model.request_customer(), "preparation does not advance clock or accept customers")
	check(not model.spend("purchase", 1, "before-income"), "spending cannot overdraw cash")
	...
	var first_income: int = model.coins
	check(first_income > 0 and model.ledger.size() == 1 and model.ledger[0].kind == "income", "payment writes one ledger entry")
	model.advance(0.1)
	check(model.coins == first_income and model.ledger.size() == 1, "customer cannot pay twice")
	...
	check(first_report.day == 1 and first_report.opening_cash == 0 and first_report.income == first_income and first_report.coins == first_income, "report reconciles opening cash and revenue")
	...
	check(not model.spend("equipment", 5, "upgrade-one") and not model.spend("wages", first_income, "too-much"), "duplicate and unaffordable spending are rejected")
	check(model.coins == first_income - 5 and model.day_reports[0].expenses.equipment == 5 and model.day_reports[0].cash_change == first_income - 5, "late expense updates finalized report")
	check(model.next_day() and not model.next_day(), "next day preparation begins only once")
	check(model.day_number == 2 and model.phase == "preopen" and model.coins == first_income - 5, "next day retains cash and advances day count")
	...
	check(second_id != first_id and model.coins > first_income - 5, "new order IDs stay unique and income accumulates")
	model.customer_departed(second_id)
	model.interact("table_1")
	model.interact("sink")
	model.advance(Day.DAY_SECONDS)
	check(model.day_reports.size() == 2 and model.day_reports[1].opening_cash == first_income - 5, "two reports keep separate opening balances")
	...
	model.new_game()
	check(model.phase == "preopen" and model.day_number == 1 and model.coins == 0 and model.day_reports.is_empty() and model.ledger.is_empty(), "new game clears all cross-day progress")
