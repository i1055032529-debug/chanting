extends SceneTree
## Render the actual preparation and day-ledger screens for visual review.
...
func _capture() -> void:
	...
	var first_error := root.get_texture().get_image().save_webp("res://docs/stage-4-1-preopen.webp", false, 0.82)
	game.start_day()
	...
	await process_frame
	await RenderingServer.frame_post_draw
	var second_error := root.get_texture().get_image().save_webp("res://docs/stage-4-1-summary.webp", false, 0.82)
	print("STAGE 4.1 PREVIEW: ", error_string(first_error), ", ", error_string(second_error))
	game.queue_free()
	await process_frame
