...
设置只影响新任务；正在搬运的食物和餐盘会先送达。主角随时能处理其他未被预留的工作。日结显示员工各类工作完成数。
...
员工领取任务时立即写入执行者，再开始走路。按优先级、顾客剩余耐心和距离选择工作；炉灶、出餐位、菜品和脏桌只允许一个执行者。玩家尝试拿已由员工领取的菜时会看到提示。员工拿到食物后走到对应餐桌；收盘后把餐盘送回回收台；清洁时走到对应污渍旁。做菜使用同一 `CookingModel`，不同菜品保留各自烹饪速度和品质结算。
...
- 规则测试覆盖做菜与取餐争抢、收盘争抢、超时餐食回收、任务中断恢复、重复结算和旧订单失效。
- 场景测试覆盖四类自动工作、任务关闭与优先级、工作中修改设置、不可达目标以及连续三个营业日；三个模拟日各接待 6 桌，均正常日结且没有残留任务预留。
- 固定客流和相同脚本化主角操作下，纯手动完成 **7 桌、流失 4 位、主角操作 50 次**；员工协作完成 **13 桌、流失 0 位、主角操作 39 次，员工完成 24 项工作**。这只是可复现的规则与节奏对比，不代表真人试玩成绩，实际游戏体验仍需进一步调参。
